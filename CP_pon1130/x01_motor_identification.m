...
thrust_g = hanging_masses - hanging_masses(1);
thrust_N = thrust_g * 9.81 / 1000;

x = linspace(0,1,100);

% Polynomial fit
poly = polyfit(control, thrust_N, 3);
y_poly = polyval(poly, x);

% Polynomial fit 
ftype = fittype('a + b*x + c*x^2 + d*x^3', 'independent', 'x');
poly = fit(control(:), thrust_N(:), ftype, 'StartPoint', [1e-6, 2 0 0]);
y_poly = poly.a + poly.b*x + poly.c*x.^2 + poly.d*x.^3;
% Exponential fit
ftype = fittype('k * x^n', 'independent', 'x');
expo = fit(control(:), thrust_N(:), ftype, 'StartPoint', [1e-6, 1]);
y_expo = expo.k*x.*expo.n;

figure;
plot(control, thrust_N, 'o', x, y_poly, '-', x, y_expo, '--' );
xlabel('Control signal u'); ylabel('Thrust [N]');
